...
app.get('/api/users/search', async (req, res) => {
  try {
    ...
    res.json(users.map(u => ({ id: u.id, name: u.name, username: u.username, photoUrl: u.photoUrl, verified: u.verified, verifyTier: u.verifyTier, initials: u.name?.slice(0,2)?.toUpperCase() })));
  } catch(e) { res.json([]); }
});
...
app.post('/api/posts', upload.array('media', 10), async (req, res) => {
  try {
    ...
    let mediaUrls = [];
    if (req.files?.length) mediaUrls = req.files.map(f => '/uploads/' + f.filename);
    if (mediaBase64) {
      const arr = JSON.parse(mediaBase64);
      for (const b64 of arr) {
        const ext = b64.startsWith('data:video') ? '.mp4' : '.jpg';
...
app.post('/api/stories', upload.single('media'), async (req, res) => {
  try {
    ...
    if (mediaBase64) {
      const ext = mediaBase64.startsWith('data:video') ? '.mp4' : '.jpg';
      const fname = uuidv4() + ext;
      const data = mediaBase64.replace(/^data:[^;]+;base64,/, '');
      fs.writeFileSync(path.join(UPLOADS_DIR, fname), Buffer.from(data, 'base64'));
  ...
  try {
    ...
    for (const m of msgs) {
      ...
      if (!seen.has(otherId)) {
        ...
        if (other) convs.push({ userId: otherId, name: other.name, username: other.username, photoUrl: other.photoUrl, verified: other.verified, verifyTier: other.verifyTier, lastMsg: m.text, lastTime: m.createdAt });
      }
    }
    res.json(convs);
  } catch(e) { res.json([]); }
...
io.on('connection', socket => {
  ...
  socket.on('webrtc_offer', ({ to, offer, callType, from }) => io.to(to).emit('webrtc_offer', { from: socket.id, offer, callType, from }));
